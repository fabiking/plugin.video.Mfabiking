...
def trailer0(params):  # Thanks to Abelhas addon!
    plugintools.log("[%s %s] PalcoTV ContextMenu (Trailer) %s" % (addonName, addonId, repr(params)))
...
def filmaff0(params):
    plugintools.log("[%s %s] PalcoTV ContextMenu (Filmaff) %s" % (addonName, addonId, repr(params)))
    show = plugintools.get_setting("pelis_view")
    plugintools.modo_vista(show)    
